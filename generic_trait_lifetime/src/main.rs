...
//     result.as_str()  // これは関数外ではもはや有効ではないのでコンパイルエラー
// }
...
// ライフタイムの省略規則
// 1. 引数はそれぞれ独自のライフタイム引数を得る
// e.g. fn foo(a: &str, b: &i32) は fn foo<'a, 'b>(a: &'a str, b: &'b i32) のように解釈される
// 2. 入力ライフタイム引数が1つ(1種類)だけなら、そのライフタイムが全ての出力ライフタイム引数に代入される
// e.g. fn foo<'a>(a: &'a str) -> &str は fn foo<'a>(a: &'a str) -> &'a str として解釈される
// 3. &selfや&mut selfが入力ライフタイム引数にあるなら、selfのライフタイムが全ての出力ライフタイム引数に代入される
// e.g. impl<'a> ImportantExcerpt<'a> { fn foo(&self) -> &str {} } は impl<'a> ImportantExcerpt<'a> { fn foo(&self) -> &'a str {} }として解釈される
...
// annは戻り値に関係ないのでライフタイム指定は不要
// TはDisplayトレイトを実装する型ならなんでもOK
// ライフタイムは一種のジェネリックなので<>内に収まっている
